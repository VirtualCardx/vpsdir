...
export const POST: APIRoute = async ({ request, redirect }) => {
  ...
  const inserted = await db
    .insert(providers)
    .values({ slug_zh, slug_en, url, category, rating, logo_key })
...
// Handle PUT and DELETE via _method query param (HTML form workaround)
export const ALL: APIRoute = async (context) => {
  const method = context.url.searchParams.get('_method');
  const id = Number(context.url.searchParams.get('id'));

  if (!id || isNaN(id)) {
    return context.redirect('/admin/?msg=error');
  }
  ...
  if (method === 'DELETE') {
    ...
    await invalidateCache(env.VPSDIR_KV, 'home:zh');
    await invalidateCache(env.VPSDIR_KV, 'home:en');
    return context.redirect('/admin/?msg=deleted');
  }
  ...
  if (method === 'PUT') {
    ...
    await invalidateCache(env.VPSDIR_KV, 'home:zh');
    await invalidateCache(env.VPSDIR_KV, 'home:en');
    return context.redirect('/admin/?msg=saved');
  }
